...
-- Helper to draw the ornate top border for "PAUSADO"
...
-- Helper to draw the ornate bottom border for "PAUSADO"
...
local function desenharBotao(btn, iconType)
    ...
    if iconType == "sword" then
        -- Sword Icon (RETORNAR AO JOGO)
        -- Crest backdrop
        love.graphics.setColor(colorCorner[1], colorCorner[2], colorCorner[3], 0.3)
        love.graphics.setLineWidth(1.5)
        drawDiamond(iconX, iconY, 20, 20, "line")
        
        -- Sword
        love.graphics.setColor(colorCorner)
        love.graphics.setLineWidth(2)
        love.graphics.line(iconX, iconY - 11, iconX, iconY + 7) -- Blade
        love.graphics.line(iconX - 6, iconY - 5, iconX + 6, iconY - 5) -- Crossguard
        love.graphics.circle("fill", iconX, iconY + 9, 2) -- Pommel
        
        -- Stylized wings/v-shape decoration
        love.graphics.line(iconX - 4, iconY + 2, iconX, iconY - 4)
        love.graphics.line(iconX + 4, iconY + 2, iconX, iconY - 4)
        
    elseif iconType == "star" then
        -- Star/Portal Rune Icon (VOLTAR AO MENU INICIAL)
...
function pausa.update(dt)
    ...
    local distVolJogo = math.sqrt((mx - handleVolJogoX)^2 + (my - (panelY + 370 + 4))^2)
    local distVolMusica = math.sqrt((mx - handleVolMusicaX)^2 + (my - (panelY + 450 + 4))^2)

    if sobreBotao or distVolJogo < 14 or distVolMusica < 14 then
    ...
    if draggingSlider then
        if love.mouse.isDown(1) then
            ...
            if draggingSlider == "volumeJogo" then
                volumeJogo = percent
...
function pausa.draw()
    ...
    -- 1. Dark overlay on screen (game visible but dimmed)
    love.graphics.setColor(0, 0, 0, 0.65)
    love.graphics.rectangle("fill", 0, 0, screenW, screenH)

    -- 2. Title "PAUSADO" and Ornaments
    desenharOrnatoSuperior(panelX, panelY + 22, panelW)

    love.graphics.setFont(fontTitle)
    -- Shadow
    love.graphics.setColor(0.12, 0.08, 0.04, 0.95)
    love.graphics.printf("PAUSADO", panelX + 2, panelY + 36, panelW, "center")
    -- Main gold text
    love.graphics.setColor(0.85, 0.70, 0.45, 1.0)
    love.graphics.printf("PAUSADO", panelX, panelY + 34, panelW, "center")

    desenharOrnatoInferior(panelX, panelY + 80, panelW)

    -- 3. Draw Buttons
    for i, btn in ipairs(botoes) do
        local iconType = "sword"
        if i == 2 then
            iconType = "star"
        elseif i == 3 then
            iconType = "exit"
        end
        desenharBotao(btn, iconType)
    end

    -- 4. Draw Volume Sliders
    local sliderW = 320
    local sliderX = panelX + 70

    -- Volume do Jogo
    desenharSlider("VOLUME DO JOGO", volumeJogo, sliderX, panelY + 370, sliderW, 8)

    -- Volume da Música
    desenharSlider("VOLUME DA MÚSICA", volumeMusica, sliderX, panelY + 450, sliderW, 8)
...
function pausa.mousepressed(x, y, button)
    if button == 1 then
        for _, btn in ipairs(botoes) do
            if btn.hover then
                btn.acao()
        ...
        -- Volume do Jogo
        if checkSliderPressed(x, y, sliderX, panelY + 370, sliderW) then
        ...
        -- Volume da Música
        elseif checkSliderPressed(x, y, sliderX, panelY + 450, sliderW) then
            draggingSlider = "volumeMusica"
            local val = (x - sliderX) / sliderW
            val = math.max(0, math.min(1, val))
            volumeMusica = math.floor(val * 100 + 0.5)
        end
    end
end
